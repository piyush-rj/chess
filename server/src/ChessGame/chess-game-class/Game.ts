import { Color, GameStatusEnum, Move, Position, PieceTypeEnum } from "../../types/websocket-types";
...
export class Game {
    ...
    public remove_player(playerId: string) {
        if (this.players.white === playerId) this.players.white = null;
        else if (this.players.black === playerId) this.players.black = null;
    }
    ...
    public restore_game_state(cached: any, moves: Move[] = []) {
        ...
        if (cached.boardState) this.restore_board_from_state(cached.boardState);
        else if (moves.length) this.replay_moves(moves);
    }
    ...
    private update_game_status() {
        ...
        if (kingInCheck && !hasMoves) this.gameStatus = GameStatusEnum.CHECKMATE;
        else if (!kingInCheck && !hasMoves) this.gameStatus = GameStatusEnum.STALEMATE;
        else if (kingInCheck) this.gameStatus = GameStatusEnum.CHECK;
        else this.gameStatus = GameStatusEnum.ACTIVE;
    }
        ...
        for (let y = 0; y < 8; y++) {
            for (let x = 0; x < 8; x++) {
                if (boardState[y] && boardState[y]![x]) {
                    const pd = boardState[y]![x];
                    const piece = Piece.create_piece(pd.color, pd.type);
                    piece.has_moved = pd.has_moved || false;
                    b[y]![x] = piece;
                } else b[y]![x] = null;
            }
        }
